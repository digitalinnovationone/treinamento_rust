...
#[get("/recursos/<id>")]
pub fn mostrar(id: u32) -> status::Custom<Result<Json<Recurso>, Json<ErroJson>>> {
    match recurso_servico::buscar_por_id(id) {
        Some(recurso) => status::Custom(Status::Ok, Ok(Json(recurso))),
        None => status::Custom(Status::NotFound, Err(Json(ErroJson{ mensagem: "Recurso não encontrado".to_string() }))),
    }
}
